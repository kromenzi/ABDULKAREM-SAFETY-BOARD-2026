...
function printPyramidReport({
  ...
  ytdCounts: Record<string, number>;
  isAr: boolean;
}) {
  const labels = isAr
    ? { title: `الهرم الأمني للمنشأة - ${year}`, subtitle: `${monthName} ${year} مقابل التراكمي YTD`, month: `الشهر: ${monthName} ${year}`, ytd: `التراكمي: يناير - ${monthName} ${year}`, leading: "المؤشرات الاستباقية", lagging: "المؤشرات التفاعلية", generated: "تم الإنشاء" }
    : { title: `Incident Pyramid - ${year}`, subtitle: `${monthName} ${year} vs YTD`, month: `Single Month: ${monthName} ${year}`, ytd: `YTD: January - ${monthName} ${year}`, leading: "Leading Indicators", lagging: "Lagging Indicators", generated: "Generated" };

  const rows = PYRAMID_LEVEL_DEFS.map((lvl) => ({
    ...lvl,
    monthCount: monthlyCounts[lvl.id] ?? 0,
    ytdCount: ytdCounts[lvl.id] ?? 0,
    label: isAr ? lvl.shortNameAr : lvl.shortNameEn
  }));

  const barWidth = (lvl: PyramidLevelDefinition) => ({
    fatality: 24, lostTime: 36, restrictedWork: 48, medicalTreatment: 60, firstAid: 72, nearMiss: 84, unsafeActs: 96
  }[lvl.id] ?? 20);

  const color = (lvl: PyramidLevelDefinition) => ({
    fatality: "#dc2626", lostTime: "#f43f5e", restrictedWork: "#f97316", medicalTreatment: "#f59e0b", firstAid: "#facc15", nearMiss: "#34d399", unsafeActs: "#059669"
  }[lvl.id] ?? "#64748b");

  const levelRows = (period: "month" | "ytd") => rows.map((lvl) => `
    <div class="level-row">
      <div class="level-bar" style="width:${barWidth(lvl)}%;background:${color(lvl)};color:${lvl.id === 'medicalTreatment' || lvl.id === 'firstAid' || lvl.id === 'nearMiss' ? '#0f172a' : '#fff'}">${escapeHtml(lvl.label)}</div>
      <div class="count">${period === 'month' ? lvl.monthCount : lvl.ytdCount}</div>
    </div>
  `).join("");
...
<style>
  ...
  * { box-sizing: border-box; }
  body { margin: 0; font-family: Arial, "Noto Sans Arabic", sans-serif; color: #0f172a; background: #fff; }
  ...
  .panel { border: 1px solid #cbd5e1; border-radius: 12px; padding: 14px; break-inside: avoid; }
  .panel h2 { margin: 0; text-align: center; font-size: 15px; font-weight: 800; color: #1e3a8a; }
  .panel p { margin: 4px 0 12px; text-align: center; font-size: 10px; color: #64748b; }
  .pyramid { display: flex; flex-direction: column; gap: 6px; min-height: 300px; justify-content: end; }
  .level-row { position: relative; min-height: 31px; display: flex; align-items: center; justify-content: center; }
  .level-bar { min-height: 31px; border-radius: 5px; display: flex; align-items: center; justify-content: center; padding: 0 8px; font-size: 10px; font-weight: 800; text-align: center; }
  .count { position: absolute; inset-inline-end: 2px; font-size: 12px; font-weight: 900; font-family: monospace; }
  .legend { margin-top: 16px; display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
  .legend-box { border: 1px solid #cbd5e1; border-radius: 10px; padding: 10px; }
  .legend-title { font-weight: 800; font-size: 11px; margin-bottom: 6px; }
  .legend-line { display:flex; justify-content:space-between; font-size:10px; margin: 3px 0; }
  .footer { margin-top: 14px; padding-top: 7px; border-top: 1px solid #cbd5e1; font-size: 8px; color: #64748b; display:flex; justify-content:space-between; gap: 12px; }
</style>
</head>
<body>
  <main class="page">
    <header class="header">
      ...
      <div class="meta">USSG ${escapeHtml(monthName)}-${year} • ${escapeHtml(labels.generated)}: ${escapeHtml(new Date().toLocaleString(isAr ? 'ar-SA' : 'en-US'))}</div>
    </header>
    <section class="grid">
      <article class="panel">
        <h2>${escapeHtml(labels.month)}</h2>
        <p>${isAr ? 'سجل الشهر المحدد' : 'Selected month records'}</p>
        <div class="pyramid">${levelRows('month')}</div>
      </article>
      <article class="panel">
        <h2>${escapeHtml(labels.ytd)}</h2>
        <p>${isAr ? 'السجل التراكمي منذ بداية العام' : 'Cumulative year-to-date records'}</p>
        <div class="pyramid">${levelRows('ytd')}</div>
      </article>
    </section>
    <section class="legend">
      <div class="legend-box">
        <div class="legend-title">${escapeHtml(labels.leading)}</div>
        <div class="legend-line"><span>${isAr ? 'أفعال/ظروف غير آمنة' : 'At-Risk / Unsafe'}</span><strong>${ytdCounts.unsafeActs ?? 0} YTD</strong></div>
        <div class="legend-line"><span>${isAr ? 'الوقائع الوشيكة' : 'Near Misses'}</span><strong>${ytdCounts.nearMiss ?? 0} YTD</strong></div>
      </div>
      <div class="legend-box">
        <div class="legend-title">${escapeHtml(labels.lagging)}</div>
        <div class="legend-line"><span>${isAr ? 'إجمالي الإصابات والحوادث' : 'Total Incident Cases'}</span><strong>${(ytdCounts.fatality ?? 0)+(ytdCounts.lostTime ?? 0)+(ytdCounts.restrictedWork ?? 0)+(ytdCounts.medicalTreatment ?? 0)+(ytdCounts.firstAid ?? 0)} YTD</strong></div>
        <div class="legend-line"><span>${isAr ? 'الحوادث الجسيمة / LTI' : 'Major / LTI Cases'}</span><strong>${(ytdCounts.fatality ?? 0)+(ytdCounts.lostTime ?? 0)} YTD</strong></div>
      </div>
    </section>
    <footer class="footer"><span>ABDULKAREM SAFETY BOARD</span><span>${escapeHtml(labels.generated)}: ${escapeHtml(new Date().toLocaleString(isAr ? 'ar-SA' : 'en-US'))}</span></footer>
  </main>
...
export function IncidentPyramid({
  ...
  const unifiedRecords = useMemo<UnifiedIncidentRecord[]>(() => {
    ...
    safetyReports.forEach((rep) => {
      ...
      const year = validDate ? dateObj.getFullYear() : selectedYear;
      const month = validDate ? dateObj.getMonth() + 1 : selectedMonth;
  ...
  const prevMonthCounts = useMemo(() => {
    ...
    return makeCounts(r => r.year === prevY && r.month === prevM);
  ...
  const handlePrintPyramid = () => {
    printPyramidReport({ year: selectedYear, month: selectedMonth, monthName: monthNameText, monthlyCounts, ytdCounts, isAr });
  };
  ...
  const getLevelDetailStats = (levelId: string, isYtd: boolean) => {
    ...
    if (mCount > pCount) trend = "up"; else if (mCount < pCount) trend = "down";
    ...
    recs.forEach((r) => { locMap[r.location] = (locMap[r.location] || 0) + 1; });
    let topLoc = isAr ? "غير محدد" : "Main Plant / Facility";
    let maxLocCount = 0;
    Object.entries(locMap).forEach(([loc, cnt]) => { if (cnt > maxLocCount) { maxLocCount = cnt; topLoc = loc; } });
    let latestDate = isAr ? "لا توجد حوادث مسجلة" : "No incidents logged";
    if (recs.length > 0) latestDate = [...recs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0].date;
    ...
    <Card className={cn("border border-border/70 shadow-sm bg-card rounded-2xl overflow-hidden incident-pyramid-container print:border-none print:shadow-none print:bg-white print:text-slate-950 print:break-inside-avoid", className)} data-testid="incident-pyramid-card">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-blue-900/10 dark:bg-blue-400/10 flex items-center justify-center text-blue-900 dark:text-blue-400"><ShieldAlert className="h-4 w-4" /></div>
              <CardTitle className="text-lg sm:text-xl font-extrabold text-blue-950 dark:text-blue-100 tracking-tight">{isAr ? `الهرم الأمني للمنشأة - ${selectedYear}` : `Incident Pyramid - ${selectedYear}`}</CardTitle>
            </div>
            <CardDescription className="text-xs text-muted-foreground mt-1">{isAr ? "مقارنة هرم السلامة الميداني للشهر الحالي بالتراكمي منذ بداية العام" : "Comparative safety pyramid for selected month vs. Year-To-Date (YTD) cumulative records"}</CardDescription>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1 bg-background border border-border/60 rounded-xl px-2 py-1 shadow-xs">
              ...
              <Select value={String(selectedMonth)} onValueChange={(val) => setSelectedMonth(Number(val))}>
                <SelectTrigger className="h-7 border-none bg-transparent text-xs font-semibold focus:ring-0 w-[90px] p-0 shadow-none"><SelectValue /></SelectTrigger>
                <SelectContent className="rounded-xl">{MONTH_NAMES.map((m, idx) => <SelectItem key={idx + 1} value={String(idx + 1)} className="text-xs">{isAr ? m.ar : m.en}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="flex items-center gap-1 bg-background border border-border/60 rounded-xl px-2 py-1 shadow-xs">
              <Select value={String(selectedYear)} onValueChange={(val) => setSelectedYear(Number(val))}>
                <SelectTrigger className="h-7 border-none bg-transparent text-xs font-semibold focus:ring-0 w-[70px] p-0 shadow-none"><SelectValue /></SelectTrigger>
                <SelectContent className="rounded-xl"><SelectItem value="2024" className="text-xs">2024</SelectItem><SelectItem value="2025" className="text-xs">2025</SelectItem><SelectItem value="2026" className="text-xs">2026</SelectItem></SelectContent>
              </Select>
            </div>
            <Button variant="outline" size="sm" onClick={handlePrintPyramid} className="h-8 rounded-xl border-border/60 text-xs gap-1.5 font-medium hover:bg-muted"><Printer className="h-3.5 w-3.5 text-blue-900 dark:text-blue-400" /><span className="hidden sm:inline">{isAr ? "طباعة" : "Print"}</span></Button>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-4 sm:p-6 space-y-6">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-8 print:gap-4 items-stretch incident-pyramid-grid">
          <div className="hidden md:block print:block absolute top-10 bottom-2 left-1/2 w-[1.5px] bg-blue-300 dark:bg-blue-800/80 print:bg-blue-400 -translate-x-1/2 z-10 incident-pyramid-divider" aria-hidden="true" />
          {[{ key: "month", title: leftSubtitle, sub: isAr ? `إحصائيات شهر ${monthNameText} ${selectedYear}` : `Single Month Records (${monthNameText})`, counts: monthlyCounts }, { key: "ytd", title: rightSubtitle, sub: isAr ? `التراكمي من يناير إلى ${monthNameText} ${selectedYear}` : `Cumulative Jan 1 - ${monthNameText} ${selectedYear}`, counts: ytdCounts }].map((panel) => (
            <div key={panel.key} className="flex flex-col items-center justify-between space-y-3 pe-0 md:pe-4">
              <div className="text-center w-full"><h3 className="text-sm sm:text-base font-bold text-blue-800 dark:text-blue-300 italic tracking-wide">{panel.title}</h3><p className="text-[11px] text-muted-foreground">{panel.sub}</p></div>
              <div className="w-full space-y-1.5 py-2 flex flex-col justify-end min-h-[310px]" role="region" aria-label={panel.title}>
                {PYRAMID_LEVEL_DEFS.map((lvl) => {
                  const count = panel.counts[lvl.id] ?? 0;
                  return <div key={`${panel.key}-${lvl.id}`} onClick={() => setActiveLevelModal({ level: lvl, isYtd: panel.key === "ytd" })} className="group relative flex items-center w-full h-8 sm:h-9 cursor-pointer transition-transform duration-200 hover:scale-[1.01]" title={`${lvl.nameEn}: ${count}`}>
                    ...
                    <div className="absolute right-0 top-0 bottom-0 flex items-center justify-start min-w-[28px] pe-1"><span className="text-xs sm:text-sm font-black font-mono text-blue-950 dark:text-blue-300 print:text-slate-950 incident-pyramid-count">{count}</span></div>
                  </div>;
                })}
              </div>
            </div>
        ...
        <div className="pt-4 border-t border-border/50 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex flex-col justify-between space-y-2"><div className="flex items-center justify-between"><span className="font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5"><CheckCircle2 className="h-4 w-4 text-emerald-600" />{isAr ? "المؤشرات الاستباقية (Leading Indicators)" : "Leading Indicators"}</span><Badge variant="outline" className="bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 font-mono text-[10px] rounded-lg">Proactive</Badge></div><div className="text-[11px] text-muted-foreground space-y-1"><div className="flex justify-between items-center"><span>• {isAr ? "السلوكيات والظروف غير الآمنة" : "At-Risk Behaviors / Unsafe Conditions"}:</span><span className="font-bold font-mono text-foreground">{monthlyCounts.unsafeActs} ({ytdCounts.unsafeActs} YTD)</span></div><div className="flex justify-between items-center"><span>• {isAr ? "الوقائع الوشيكة (Near Miss)" : "Near Misses"}:</span><span className="font-bold font-mono text-foreground">{monthlyCounts.nearMiss} ({ytdCounts.nearMiss} YTD)</span></div></div></div>
          <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 flex flex-col justify-between space-y-2"><div className="flex items-center justify-between"><span className="font-bold text-amber-900 dark:text-amber-300 flex items-center gap-1.5"><ShieldAlert className="h-4 w-4 text-amber-600" />{isAr ? "المؤشرات التفاعلية (Lagging Indicators)" : "Lagging Indicators"}</span><Badge variant="outline" className="bg-amber-500/20 text-amber-800 dark:text-amber-300 font-mono text-[10px] rounded-lg">Reactive</Badge></div><div className="text-[11px] text-muted-foreground space-y-1"><div className="flex justify-between items-center"><span>• {isAr ? "إجمالي الإصابات والحوادث (LTI/MTC/FAC)" : "Total Incident Cases"}:</span><span className="font-bold font-mono text-foreground">{monthlyCounts.fatality + monthlyCounts.lostTime + monthlyCounts.restrictedWork + monthlyCounts.medicalTreatment + monthlyCounts.firstAid} ({ytdCounts.fatality + ytdCounts.lostTime + ytdCounts.restrictedWork + ytdCounts.medicalTreatment + ytdCounts.firstAid} YTD)</span></div><div className="flex justify-between items-center"><span>• {isAr ? "الحوادث الجسيمة (Fatal / LTI)" : "Major / LTI Cases"}:</span><span className="font-bold font-mono text-foreground">{monthlyCounts.fatality + monthlyCounts.lostTime} ({ytdCounts.fatality + ytdCounts.lostTime} YTD)</span></div></div></div>
        </div>
      </CardContent>
