...
const SAMPLE_INCIDENTS: IncidentRecord[] = [
  {
    ...
    refNo: "INC-2024-101",
    type: "Near Miss",
    title: "Unsecured Scaffold Board Slipped Near High Bay Rack",
    date: "2024-05-18",
    time: "11:15 AM",
    location: "High Bay Storage Warehouse B",
    department: "Logistics",
    factory: "Main Factory 1",
    reportedBy: "Tariq Mansoor",
    severity: "Medium",
...
export default function AdminIncidentsPage() {
  ...
  const filtered = incidents.filter((inc) => {
    const matchesSearch = inc.title.toLowerCase().includes(searchTerm.toLowerCase()) || inc.refNo.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-6" data-testid="admin-incidents-page">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-[28px] font-bold tracking-tight">
              {isAr ? "إدارة الحوادث والوقائع الوشيكة (Near Miss)" : "Incidents & Near Miss Management"}
            </h2>
            <p className="text-[12px] text-muted-foreground">
              {isAr ? "تحليل السبب الجذر (5 Why & Fishbone)، الخطة التصحيحية والدروس المستفادة" : "Root cause investigation (5-Why & Fishbone), corrective action tracking & lessons learned"}
            </p>
          </div>
        </div>
        ...
        <Button
          onClick={() => {
            const newInc: IncidentRecord = {
              ...
              factory: "Main Factory 1",
              reportedBy: "Abdulkarem Alanzi",
              severity: "Medium",
              description: "",
              fiveWhys: [
                { whyNo: 1, question: "Why did it happen?", answer: "" },
                { whyNo: 2, question: "Why?", answer: "" },
                { whyNo: 3, question: "Why?", answer: "" },
                { whyNo: 4, question: "Why?", answer: "" },
                { whyNo: 5, question: "Root Cause?", answer: "" }
              ],
        ...
        >
          ...
          {isAr ? "تسجيل حادث / وشيكة جديد" : "Report Incident / Near Miss"}
        </Button>
      </div>
      ...
      <Card className="p-4 space-y-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder={isAr ? "البحث بالرقم المرجعي، العنوان أو الموقع..." : "Search incidents..."}
              value={searchTerm}
          ...
          <Select value={typeFilter} onValueChange={setTypeFilter}>
            <SelectTrigger className="w-[200px]">
              <SelectValue placeholder={isAr ? "نوع الحادث" : "Incident Type"} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{isAr ? "جميع الانواع" : "All Types"}</SelectItem>
              <SelectItem value="Near Miss">Near Miss</SelectItem>
              <SelectItem value="First Aid">First Aid</SelectItem>
              <SelectItem value="Lost Time Injury">Lost Time Injury (LTI)</SelectItem>
          ...
          <Table>
            <TableHeader className="bg-muted/50">
              <TableRow>
                ...
                <TableHead>{isAr ? "النوع والعنوان" : "Type & Title"}</TableHead>
                <TableHead>{isAr ? "الموقع والقسم" : "Location & Dept"}</TableHead>
                <TableHead>{isAr ? "التاريخ والخطورة" : "Date & Severity"}</TableHead>
              ...
              {filtered.map((inc) => (
                <TableRow key={inc.id}>
                  ...
                  <TableCell>
                    ...
                    <p className="text-[11px] text-muted-foreground">{inc.department}</p>
                  </TableCell>
                  <TableCell>
      ...
      {activeIncident && isDialogOpen && (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto">
            ...
            <Tabs defaultValue="overview" className="w-full mt-2 print-all-tabs">
              <TabsList className="grid grid-cols-4 w-full">
                <TabsTrigger value="overview">{isAr ? "الوصف العام" : "Overview"}</TabsTrigger>
                <TabsTrigger value="fivewhy">{isAr ? "تحليل 5 Why" : "5-Why RCA"}</TabsTrigger>
                <TabsTrigger value="fishbone">{isAr ? "عظم السمكة (Ishikawa)" : "Fishbone"}</TabsTrigger>
                <TabsTrigger value="actions">{isAr ? "الإجراءات والدروس" : "Actions & Lessons"}</TabsTrigger>
              </TabsList>
